...
tic, x1 = decomposicao_LU(M,Pb); tempo_lu = toc;
tic, x2 = decomposicao_cholesky(M,Pb); tempo_ch = toc;

x0 = zeros(n,1);

tic, i1 = decomposicao_jacobi(M,Pb,x0); tempo_ja = toc;
tic, i2 = decomposicao_gaussseidel(M,Pb,x0); tempo_gs = toc;
tic, i3 = decomposicao_gradientes(M,Pb,x0); tempo_gr = toc;
...
function i = decomposicao_jacobi(M,Pb,x0)
    n = size(M, 1);
    D = diag(diag(M));
    C = eye(n) - D \ M;
    g = D \ Pb;
...
function i = decomposicao_gaussseidel(M,Pb,x0)
    INF = tril(M);
    SUP = triu(M,1);
...
function i = decomposicao_gradientes(M,Pb,x0)
    r0 = M*x0-Pb;
    p0 = -r0;
    alfa = r0'*r0/(r0'*M*r0);
    x = x0+alfa*p0;
